...
class Tray():
    ...
    def create_matrix(self, only_decap):
        ...
        if only_decap:
            ...
            for i in range(self.syr_batch, self.syr_batch+self.Lpen_batch):
                for j in range(self.Lpen_samp):
                    matrix[i][j] = LPen() 
            for i in range(self.syr_batch+self.Lpen_batch, self.syr_batch+self.Lpen_batch+self.Spen_batch):
